...
def mock_operations(func):
    def wrapper(self):
        ...
        self.assertIsNone(ibm_schematics_job_info.MISSING_IMPORT_EXC)

        # Set-up mocks for each operation.
        self.read_patcher = patch('plugins.modules.ibm_schematics_job_info.SchematicsV1.get_job')
...
class TestJobModuleInfo(ModuleTestCase):
    ...
    def test_read_ibm_schematics_job_failed(self):
        ...
        self.read_mock.side_effect = ApiException(400, message='Read ibm_schematics_job error')
        ...
        self.assertEqual(result.exception.args[0]['msg'], 'Read ibm_schematics_job error')
